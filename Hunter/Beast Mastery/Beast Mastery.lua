...
--######################################
-- Edited by TheAllSeeing, see changelog for more details.
-- v1.1
--[[
Borrowed icons:
...
-- Passes unitID to function and returns if unit is a boss or not (true or false)
local function unitIsBoss(unitID)
    local unitIsBoss = Unit(unitID):IsBoss()
    return unitIsBoss
end
    ...
    local function EnemyRotation(unitID)
        ...
        if activeEnemies < 2 or not A.BeastCleave:IsTalentLearned() and activeEnemies < 3 then
            ...
            if A.BestialWrath:IsReady(player) then
                if BurstIsON(unitID) and (unitTimeToDie(unitID) < 16 and unitIsBoss(unitID) or unitTimeToDie(unitID) > 16) and A.BarbedShot:IsInRange(unitID) then
