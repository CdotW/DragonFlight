...
Action[ACTION_CONST_HUNTER_BEASTMASTERY] = {
    ...
    BloodyFrenzy         		= Action.Create({ Type = "Spell", ID = 407412, Hidden = true   }),
}
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unitID)
        ...
        if activeEnemies < 2 or not A.BeastCleave:IsTalentLearned() and activeEnemies < 3 then
            ...
            if A.CalloftheWild:IsReady(player) and (Unit(unitID):IsBoss() or Unit(unitID):TimeToDie() > 16) and BurstIsON(unitID) then
                return A.CalloftheWild:Show(icon)
            end

            -- actions.st+=/stampede
            if A.Stampede:IsReady(player) and Unit(unitID):TimeToDie() > 10 then
                return A.Stampede:Show(icon)
            end

            -- actions.st+=/bloodshed
            if A.Bloodshed:IsReady(unitID) and Unit(unitID):TimeToDie() > 10 then
                return A.Bloodshed:Show(icon)
            end

            -- actions.st+=/bestial_wrath
            if A.BestialWrath:IsReady(player) and BurstIsON(unitID) and (Unit(unitID):IsBoss() or Unit(unitID):TimeToDie() > 16) and A.BarbedShot:IsInRange(unitID) then
                return A.BestialWrath:Show(icon)
            end

            -- actions.st+=/death_chakram
            if A.DeathChakram:IsReady(unitID) and Unit(unitID):TimeToDie() > 10 then
            ...
            if A.AMurderOfCrows:IsReady(unitID) and Unit(unitID):TimeToDie() > 10 then
            ...
            if A.ExplosiveShot:IsReady(unitID) and Unit(unitID):TimeToDie() > 10 then
            ...
            if A.DireBeast:IsReady(unitID) and Unit(unitID):TimeToDie() > 10 then
        ...
        if activeEnemies > 2 or A.BeastCleave:IsTalentLearned() and activeEnemies > 1 then
            ...
            if A.MultiShot:IsReady(unitID) and (Unit(player):HasBuffs(A.BeastCleaveBuff.ID) < (0.25 + A.GetGCD()) and (not A.BloodyFrenzy:IsTalentLearned() or A.CalloftheWild:GetCooldown() > 0)) then
            ...
            if A.CalloftheWild:IsReady(player) and BurstIsON(unitID) and (Unit(unitID):IsBoss() or Unit(unitID):TimeToDie() > 16) then
              return A.CalloftheWild:Show(icon)
            end
            ...
            if A.Barrage:IsReady(player) and Unit(pet):HasBuffs(A.Frenzy.ID) > A.Barrage:GetSpellCastTime() and Unit(unitID):TimeToDie() > 10 then
            ...
            if A.KillShot:IsReady(player) then
              return A.KillShot:Show(icon)
            end

            -- actions.cleave+=/cobra_shot,if=focus.time_to_max<gcd*2
            if A.CobraShot:IsReady(player) and Player:FocusTimeToMax() < A.GetGCD() * 2 then
              return A.CobraShot:Show(icon)
            end
